...
\- テンプレシート（sheetId: 1198447577）: 成果レポートのひな形
...
3. **ラベル別サマリをシートに書き込む**
   ...
   - write_report.sh がラベル数 > テンプレ行数の場合に示唆前へ行挿入済みのため、追加挿入は不要
...
| C4 | 配信対象期間テキスト |
| B7 | 「分析結果」ラベル |
| F9:{動的}9 | 分析結果ヘッダー行（列数はconfig.jsonのsummaryColsに従う） |
| F10:{動的}〜 | 月別データ行（月数分） |
| B15 | 「示唆」ラベル |
| C17 | 示唆本文（起点。実際の行はINSIGHT_CELLに従う） |
| A20:I〜 | スクラッチエリア（集計数式の一時置き場） |
