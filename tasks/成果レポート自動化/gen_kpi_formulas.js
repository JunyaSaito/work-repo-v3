...
const dateFilter = `'配信実績'!$${dateCol}$2:$${dateCol},">="&${ds},'配信実績'!$${dateCol}$2:$${dateCol},"<="&${de}`;

// 施策数(月間): 期間内のユニーク施策数 / 月数
const f_count = `=COUNTA(UNIQUE(FILTER('配信実績'!$${campaignCol}$2:$${campaignCol},'配信実績'!$${dateCol}$2:$${dateCol}>=${ds},'配信実績'!$${dateCol}$2:$${dateCol}<=${de})))/${nMonths}`;

// CV数(月間): CV系指標の合計 / 月数
const cvMetrics = config.metrics.filter(
  (m) => (m.summaryName || m.name).includes("CV数")
);
...
const amountMetrics = config.metrics.filter(
  (m) => (m.summaryName || m.name).includes("CV金額")
);
const amountSumifs = amountMetrics
  .map((m) => `SUMIFS('配信実績'!$${m.srcCol}$2:$${m.srcCol},${dateFilter})`)
  .join("+");
const f_amount = amountSumifs ? `=(${amountSumifs})/${nMonths}` : "";
